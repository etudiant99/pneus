...
class Pneus_Widget extends WP_Widget{
    ...
    public function widget($args, $instance){
        ...
        <form method="post">
            <p>
		      <label>Pneu :</label>
              <select  id="pneu" name="pneu" style="width:100%;margin-top:0px;margin-bottom: 10px">
                <?php
    ...
    {
        if (isset($_GET['pneu']) && !empty($_GET['pneu'])) {
            global $wpdb;
            $marque = $_GET['pneu'];

            $allItems = $wpdb->get_results("SELECT * FROM wp_inventaire WHERE pneu = '$pneu'");
